...
class Decoder(torch.nn.Module):
    ...
    def decoder_prediction(self, encoded: torch.Tensor, encoded_future: torch.Tensor, previous_power: torch.Tensor, known_targets: torch.Tensor | None = None) -> torch.Tensor:
        self._target_rnn_cell.setup_memory(encoded)
        ...
        for i in range(horizon):
            ...
            if i < horizon - 1:
                has_real_value = (known_targets is not None and not torch.isnan(known_targets[:, i, :]).any())

                if has_real_value:
                    power_for_next_step = known_targets[:, i, :]
                else:
                    power_for_next_step = predictions

                meteo_now = encoded_future[:, i + 1, :]
                input = torch.cat([power_for_next_step, meteo_now], dim=-1)
...
class Model(torch.nn.Module): 
    ...
    def forward(self, x_past: torch.Tensor, x_future: torch.Tensor, targets: torch.Tensor | None = None, known_targets: torch.Tensor | None = None, curr_epoch: int = 0) -> torch.Tensor:
        ...
        if targets is not None:
            ...
            return self.decoder.decoder_prediction(encoded, encoded_future, previous_power, known_targets=known_targets)